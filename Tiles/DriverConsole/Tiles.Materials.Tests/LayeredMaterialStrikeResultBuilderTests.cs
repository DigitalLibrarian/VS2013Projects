...
namespace Tiles.Materials.Tests
{
    ...
    public class LayeredMaterialStrikeResultBuilderTests
    {
        ...
        public void Blunt_CompleteFracture_TempEdgedConversion()
        {
            ...
            {
                IsDefeated = true,
                StressResult = StressResult.Impact_CompleteFracture,
                ResultMomentum = momentum
            };
            ...
            var layerResult2 = new MaterialStrikeResult
            {
                IsDefeated = false,
                ResultMomentum = momentum,
                StressResult = StressResult.None
            };
            SetupLayerResponse(layerResult0, layerResult1, layerResult2);
            ...
            Assert.AreEqual(Layer1.Thickness, result.Penetration);
            Assert.AreEqual(3, result.LayerResults.Count());
            Assert.AreSame(layerResult0, result.LayerResults.ElementAt(0));
            Assert.AreSame(layerResult1, result.LayerResults.ElementAt(1));
            Assert.AreSame(layerResult2, result.LayerResults.ElementAt(2));
            Assert.AreEqual(3, result.TaggedResults.Count());
            Assert.IsTrue(result.TaggedResults.ContainsKey(Layer0.Tag));
            Assert.AreEqual(layerResult0, result.TaggedResults[Layer0.Tag]);
            ...
            Assert.IsTrue(result.TaggedResults.ContainsKey(Layer2.Tag));
            Assert.AreEqual(layerResult2, result.TaggedResults[Layer2.Tag]);
            ...
            LayerTesterMock.Verify(x => x.StrikeTest(
                ...
                strikerContactArea,
                layerResult0.ResultMomentum,
                penetrationLeft - Layer1.Thickness,
                Layer2.Material,
                Layer2.Thickness,
                Layer2.Volume,
                strickenContactArea
